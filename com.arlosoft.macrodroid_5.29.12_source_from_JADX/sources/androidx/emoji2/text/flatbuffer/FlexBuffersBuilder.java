...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    public final ReadWriteBuf f537bb;
    ...
    private int align(int i) {
        int i2 = 1 << i;
        int access$100 = Value.paddingBytes(this.f537bb.writePosition(), i2);
        while (true) {
            int i3 = access$100 - 1;
            if (access$100 == 0) {
                return i2;
            }
            this.f537bb.put((byte) 0);
            access$100 = i3;
        }
    }
    ...
    private Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            i3++;
            max = Math.max(max, Value.elemWidth(4, 0, (long) this.stack.get(i3).key, this.f537bb.writePosition(), i3));
        }
        int align = align(max);
        writeInt(j, align);
        int writePosition = this.f537bb.writePosition();
    ...
    private Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        int max = Math.max(0, widthUInBits(j));
        if (value2 != null) {
            max = Math.max(max, value2.elemWidth(this.f537bb.writePosition(), 0));
            i4 = 3;
        } else {
            i4 = 1;
        }
        ...
        for (int i9 = i2; i9 < this.stack.size(); i9++) {
            i8 = Math.max(i8, this.stack.get(i9).elemWidth(this.f537bb.writePosition(), i9 + i4));
            int i10 = i2;
            if (z && i9 == i10) {
        ...
        if (!z) {
            while (i11 < this.stack.size()) {
                this.f537bb.put(this.stack.get(i11).storedPackedType(i8));
                i11++;
            }
        }
        if (value2 != null) {
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) != 0) {
            ...
            this.f537bb.put(bytes, 0, bytes.length);
            this.f537bb.put((byte) 0);
            this.keyPool.put(str, Integer.valueOf(writePosition));
            return writePosition;
        }
        byte[] bytes2 = str.getBytes(StandardCharsets.UTF_8);
        this.f537bb.put(bytes2, 0, bytes2.length);
        this.f537bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(writePosition));
        return writePosition;
    }
    ...
    private Value writeBlob(int i, byte[] bArr, int i2, boolean z) {
        ...
        int writePosition = this.f537bb.writePosition();
        this.f537bb.put(bArr, 0, bArr.length);
        if (z) {
            this.f537bb.put((byte) 0);
        }
    ...
    private void writeDouble(double d, int i) {
        if (i == 4) {
            this.f537bb.putFloat((float) d);
        } else if (i == 8) {
            this.f537bb.putDouble(d);
        }
    }

    private void writeInt(long j, int i) {
        if (i == 1) {
            this.f537bb.put((byte) ((int) j));
        } else if (i == 2) {
            this.f537bb.putShort((short) ((int) j));
        } else if (i == 4) {
            this.f537bb.putInt((int) j);
        } else if (i == 8) {
            this.f537bb.putLong(j);
        }
    }

    private void writeOffset(long j, int i) {
        writeInt((long) ((int) (((long) this.f537bb.writePosition()) - j)), i);
    }
    ...
    public ByteBuffer finish() {
        int align = align(this.stack.get(0).elemWidth(this.f537bb.writePosition(), 0));
        writeAny(this.stack.get(0), align);
        this.f537bb.put(this.stack.get(0).storedPackedType());
        this.f537bb.put((byte) align);
        this.finished = true;
        return ByteBuffer.wrap(this.f537bb.data(), 0, this.f537bb.writePosition());
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>() {
            public int compare(Value value, Value value2) {
                ...
                int i2 = value2.key;
                do {
                    b = FlexBuffersBuilder.this.f537bb.get(i);
                    b2 = FlexBuffersBuilder.this.f537bb.get(i2);
                    if (b == 0) {
                        return b - b2;
                    }
                    i++;
