            ...
            if (workSpec.state == WorkInfo.State.ENQUEUED) {
                if (currentTimeMillis < calculateNextRunTime) {
                    ...
                    if (i >= 23 && workSpec.constraints.requiresDeviceIdle()) {
                        ...
                        hashSet2.add(workSpec.f709id);
                    } else {
                        Logger.get().debug(TAG, String.format("Ignoring WorkSpec %s, Requires ContentUri triggers.", new Object[]{workSpec}), new Throwable[0]);
                    }
                } else {
                    Logger.get().debug(TAG, String.format("Starting work for %s", new Object[]{workSpec.f709id}), new Throwable[0]);
                    this.mWorkManagerImpl.startWork(workSpec.f709id);
                }
            }
        }
        synchronized (this.mLock) {
