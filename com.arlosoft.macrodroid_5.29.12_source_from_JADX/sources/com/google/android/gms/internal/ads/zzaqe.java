package com.google.android.gms.internal.ads;

import android.content.Context;
import android.util.Pair;
import androidx.annotation.VisibleForTesting;
import com.google.android.gms.ads.identifier.AdvertisingIdClient;
import com.google.android.gms.common.GooglePlayServicesNotAvailableException;
import com.google.android.gms.common.GooglePlayServicesRepairableException;
import dalvik.system.DexClassLoader;
import java.io.File;
import java.io.IOException;
import java.lang.reflect.Method;
import java.util.HashMap;
import java.util.Map;
import java.util.concurrent.ExecutionException;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.Future;
import java.util.concurrent.TimeUnit;
import java.util.concurrent.TimeoutException;

/* compiled from: com.google.android.gms:play-services-ads@@21.3.0 */
public final class zzaqe {

    /* renamed from: a */
    protected final Context f25290a;

    /* renamed from: b */
    private ExecutorService f25291b;

    /* renamed from: c */
    private DexClassLoader f25292c;

    /* renamed from: d */
    private zzapk f25293d;

    /* renamed from: e */
    private byte[] f25294e;

    /* renamed from: f */
    private volatile AdvertisingIdClient f25295f = null;

    /* renamed from: g */
    private volatile boolean f25296g;

    /* renamed from: h */
    private Future f25297h;

    /* renamed from: i */
    private final boolean f25298i;
    /* access modifiers changed from: private */

    /* renamed from: j */
    public volatile zzana f25299j;

    /* renamed from: k */
    private Future f25300k;

    /* renamed from: l */
    private zzaoz f25301l;

    /* renamed from: m */
    protected boolean f25302m;

    /* renamed from: n */
    protected boolean f25303n;

    /* renamed from: o */
    private final Map f25304o;

    /* renamed from: p */
    private boolean f25305p;

    /* renamed from: q */
    private zzapx f25306q;

    private zzaqe(Context context) {
        boolean z = false;
        this.f25296g = false;
        this.f25297h = null;
        this.f25299j = null;
        this.f25300k = null;
        this.f25302m = false;
        this.f25303n = false;
        this.f25305p = false;
        Context applicationContext = context.getApplicationContext();
        this.f25298i = applicationContext != null ? true : z;
        context = applicationContext != null ? applicationContext : context;
        this.f25290a = context;
        this.f25304o = new HashMap();
        if (this.f25306q == null) {
            this.f25306q = new zzapx(context);
        }
    }

    /* JADX WARNING: Can't wrap try/catch for region: R(17:1|2|(1:4)|5|6|7|8|(1:10)(1:11)|12|(1:14)(1:15)|16|17|18|(2:20|(1:22)(2:23|24))|25|26|(3:27|28|(17:30|(2:32|33)|34|35|36|37|(2:39|(1:41)(2:42|43))|44|(1:46)|47|48|49|50|51|52|53|82)(3:71|72|73))) */
    /* JADX WARNING: Missing exception handler attribute for start block: B:17:0x004f */
    /* JADX WARNING: Removed duplicated region for block: B:20:0x0058 A[Catch:{ zzapv -> 0x0165 }] */
    /* JADX WARNING: Removed duplicated region for block: B:30:0x0086 A[Catch:{ IllegalArgumentException -> 0x0157, zzapj -> 0x015e }] */
    /* JADX WARNING: Removed duplicated region for block: B:71:0x0151 A[SYNTHETIC, Splitter:B:71:0x0151] */
    /* renamed from: g */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static com.google.android.gms.internal.ads.zzaqe m42100g(android.content.Context r9, java.lang.String r10, java.lang.String r11, boolean r12) {
        /*
            java.lang.String r10 = "%s/%s.dex"
            java.lang.String r11 = "1658186039475"
            com.google.android.gms.internal.ads.zzaqe r0 = new com.google.android.gms.internal.ads.zzaqe
            r0.<init>(r9)
            com.google.android.gms.internal.ads.zzaqa r9 = new com.google.android.gms.internal.ads.zzaqa     // Catch:{ zzapv -> 0x0165 }
            r9.<init>()     // Catch:{ zzapv -> 0x0165 }
            java.util.concurrent.ExecutorService r9 = java.util.concurrent.Executors.newCachedThreadPool(r9)     // Catch:{ zzapv -> 0x0165 }
            r0.f25291b = r9     // Catch:{ zzapv -> 0x0165 }
            r0.f25296g = r12     // Catch:{ zzapv -> 0x0165 }
            if (r12 == 0) goto L_0x0025
            java.util.concurrent.ExecutorService r9 = r0.f25291b     // Catch:{ zzapv -> 0x0165 }
            com.google.android.gms.internal.ads.zzaqb r12 = new com.google.android.gms.internal.ads.zzaqb     // Catch:{ zzapv -> 0x0165 }
            r12.<init>(r0)     // Catch:{ zzapv -> 0x0165 }
            java.util.concurrent.Future r9 = r9.submit(r12)     // Catch:{ zzapv -> 0x0165 }
            r0.f25297h = r9     // Catch:{ zzapv -> 0x0165 }
        L_0x0025:
            java.util.concurrent.ExecutorService r9 = r0.f25291b     // Catch:{ zzapv -> 0x0165 }
            com.google.android.gms.internal.ads.zzaqd r12 = new com.google.android.gms.internal.ads.zzaqd     // Catch:{ zzapv -> 0x0165 }
            r12.<init>(r0)     // Catch:{ zzapv -> 0x0165 }
            r9.execute(r12)     // Catch:{ zzapv -> 0x0165 }
            r9 = 1
            r12 = 0
            com.google.android.gms.common.GoogleApiAvailabilityLight r1 = com.google.android.gms.common.GoogleApiAvailabilityLight.m3542h()     // Catch:{ all -> 0x004f }
            android.content.Context r2 = r0.f25290a     // Catch:{ all -> 0x004f }
            int r2 = r1.mo21171b(r2)     // Catch:{ all -> 0x004f }
            if (r2 <= 0) goto L_0x003f
            r2 = 1
            goto L_0x0040
        L_0x003f:
            r2 = 0
        L_0x0040:
            r0.f25302m = r2     // Catch:{ all -> 0x004f }
            android.content.Context r2 = r0.f25290a     // Catch:{ all -> 0x004f }
            int r1 = r1.mo21156i(r2)     // Catch:{ all -> 0x004f }
            if (r1 != 0) goto L_0x004c
            r1 = 1
            goto L_0x004d
        L_0x004c:
            r1 = 0
        L_0x004d:
            r0.f25303n = r1     // Catch:{ all -> 0x004f }
        L_0x004f:
            r0.mo41816o(r12, r9)     // Catch:{ zzapv -> 0x0165 }
            boolean r1 = com.google.android.gms.internal.ads.zzaqh.m42133f()     // Catch:{ zzapv -> 0x0165 }
            if (r1 == 0) goto L_0x0073
            com.google.android.gms.internal.ads.zzbiu r1 = com.google.android.gms.internal.ads.zzbjc.f27294x2     // Catch:{ zzapv -> 0x0165 }
            com.google.android.gms.internal.ads.zzbja r2 = com.google.android.gms.ads.internal.client.zzay.m1924c()     // Catch:{ zzapv -> 0x0165 }
            java.lang.Object r1 = r2.mo42663b(r1)     // Catch:{ zzapv -> 0x0165 }
            java.lang.Boolean r1 = (java.lang.Boolean) r1     // Catch:{ zzapv -> 0x0165 }
            boolean r1 = r1.booleanValue()     // Catch:{ zzapv -> 0x0165 }
            if (r1 != 0) goto L_0x006b
            goto L_0x0073
        L_0x006b:
            java.lang.IllegalStateException r9 = new java.lang.IllegalStateException     // Catch:{ zzapv -> 0x0165 }
            java.lang.String r10 = "Task Context initialization must not be called from the UI thread."
            r9.<init>(r10)     // Catch:{ zzapv -> 0x0165 }
            throw r9     // Catch:{ zzapv -> 0x0165 }
        L_0x0073:
            com.google.android.gms.internal.ads.zzapk r1 = new com.google.android.gms.internal.ads.zzapk     // Catch:{ zzapv -> 0x0165 }
            r2 = 0
            r1.<init>(r2)     // Catch:{ zzapv -> 0x0165 }
            r0.f25293d = r1     // Catch:{ zzapv -> 0x0165 }
            java.lang.String r3 = "HeBkX9XaSpC6sV82I6X2HUgm82vH8VhIWt26LGkrI3A="
            byte[] r3 = com.google.android.gms.internal.ads.zzanp.m41982b(r3, r12)     // Catch:{ IllegalArgumentException -> 0x0157 }
            int r4 = r3.length     // Catch:{ IllegalArgumentException -> 0x0157 }
            r5 = 32
            if (r4 != r5) goto L_0x0151
            r4 = 4
            r5 = 16
            java.nio.ByteBuffer r3 = java.nio.ByteBuffer.wrap(r3, r4, r5)     // Catch:{ IllegalArgumentException -> 0x0157 }
            byte[] r4 = new byte[r5]     // Catch:{ IllegalArgumentException -> 0x0157 }
            r3.get(r4)     // Catch:{ IllegalArgumentException -> 0x0157 }
            r3 = 0
        L_0x0093:
            if (r3 >= r5) goto L_0x009f
            byte r6 = r4[r3]     // Catch:{ IllegalArgumentException -> 0x0157 }
            r6 = r6 ^ 68
            byte r6 = (byte) r6     // Catch:{ IllegalArgumentException -> 0x0157 }
            r4[r3] = r6     // Catch:{ IllegalArgumentException -> 0x0157 }
            int r3 = r3 + 1
            goto L_0x0093
        L_0x009f:
            r0.f25294e = r4     // Catch:{ zzapj -> 0x015e }
            android.content.Context r1 = r0.f25290a     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.io.File r1 = r1.getCacheDir()     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            if (r1 != 0) goto L_0x00ba
            android.content.Context r1 = r0.f25290a     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.String r3 = "dex"
            java.io.File r1 = r1.getDir(r3, r12)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            if (r1 == 0) goto L_0x00b4
            goto L_0x00ba
        L_0x00b4:
            com.google.android.gms.internal.ads.zzapv r9 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r9.<init>()     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            throw r9     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
        L_0x00ba:
            java.lang.String r3 = ""
            java.io.File r4 = new java.io.File     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r5 = 2
            java.lang.Object[] r6 = new java.lang.Object[r5]     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r6[r12] = r1     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r6[r9] = r11     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.String r7 = "%s/%s.jar"
            java.lang.String r6 = java.lang.String.format(r7, r6)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r4.<init>(r6)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            boolean r6 = r4.exists()     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            if (r6 != 0) goto L_0x00eb
            com.google.android.gms.internal.ads.zzapk r6 = r0.f25293d     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            byte[] r7 = r0.f25294e     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            byte[] r3 = r6.mo41785b(r7, r3)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r4.createNewFile()     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.io.FileOutputStream r6 = new java.io.FileOutputStream     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r6.<init>(r4)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            int r7 = r3.length     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r6.write(r3, r12, r7)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r6.close()     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
        L_0x00eb:
            r0.m42105x(r1, r11)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            dalvik.system.DexClassLoader r3 = new dalvik.system.DexClassLoader     // Catch:{ all -> 0x0120 }
            java.lang.String r6 = r4.getAbsolutePath()     // Catch:{ all -> 0x0120 }
            java.lang.String r7 = r1.getAbsolutePath()     // Catch:{ all -> 0x0120 }
            android.content.Context r8 = r0.f25290a     // Catch:{ all -> 0x0120 }
            java.lang.ClassLoader r8 = r8.getClassLoader()     // Catch:{ all -> 0x0120 }
            r3.<init>(r6, r7, r2, r8)     // Catch:{ all -> 0x0120 }
            r0.f25292c = r3     // Catch:{ all -> 0x0120 }
            m42106y(r4)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r0.m42104w(r1, r11)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.Object[] r2 = new java.lang.Object[r5]     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r2[r12] = r1     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r2[r9] = r11     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.String r10 = java.lang.String.format(r10, r2)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            m42107z(r10)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            com.google.android.gms.internal.ads.zzaoz r10 = new com.google.android.gms.internal.ads.zzaoz     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r0)     // Catch:{ zzapv -> 0x0165 }
            r0.f25301l = r10     // Catch:{ zzapv -> 0x0165 }
            r0.f25305p = r9     // Catch:{ zzapv -> 0x0165 }
            goto L_0x0165
        L_0x0120:
            r2 = move-exception
            m42106y(r4)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r0.m42104w(r1, r11)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.Object[] r3 = new java.lang.Object[r5]     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r3[r12] = r1     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            r3[r9] = r11     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            java.lang.String r9 = java.lang.String.format(r10, r3)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            m42107z(r9)     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
            throw r2     // Catch:{ FileNotFoundException -> 0x014a, IOException -> 0x0143, zzapj -> 0x013c, NullPointerException -> 0x0135 }
        L_0x0135:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapv r10 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r9)     // Catch:{ zzapv -> 0x0165 }
            throw r10     // Catch:{ zzapv -> 0x0165 }
        L_0x013c:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapv r10 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r9)     // Catch:{ zzapv -> 0x0165 }
            throw r10     // Catch:{ zzapv -> 0x0165 }
        L_0x0143:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapv r10 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r9)     // Catch:{ zzapv -> 0x0165 }
            throw r10     // Catch:{ zzapv -> 0x0165 }
        L_0x014a:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapv r10 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r9)     // Catch:{ zzapv -> 0x0165 }
            throw r10     // Catch:{ zzapv -> 0x0165 }
        L_0x0151:
            com.google.android.gms.internal.ads.zzapj r9 = new com.google.android.gms.internal.ads.zzapj     // Catch:{ IllegalArgumentException -> 0x0157 }
            r9.<init>(r1)     // Catch:{ IllegalArgumentException -> 0x0157 }
            throw r9     // Catch:{ IllegalArgumentException -> 0x0157 }
        L_0x0157:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapj r10 = new com.google.android.gms.internal.ads.zzapj     // Catch:{ zzapj -> 0x015e }
            r10.<init>(r1, r9)     // Catch:{ zzapj -> 0x015e }
            throw r10     // Catch:{ zzapj -> 0x015e }
        L_0x015e:
            r9 = move-exception
            com.google.android.gms.internal.ads.zzapv r10 = new com.google.android.gms.internal.ads.zzapv     // Catch:{ zzapv -> 0x0165 }
            r10.<init>(r9)     // Catch:{ zzapv -> 0x0165 }
            throw r10     // Catch:{ zzapv -> 0x0165 }
        L_0x0165:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzaqe.m42100g(android.content.Context, java.lang.String, java.lang.String, boolean):com.google.android.gms.internal.ads.zzaqe");
    }

    /* access modifiers changed from: private */
    /* renamed from: v */
    public final void m42103v() {
        try {
            if (this.f25295f == null && this.f25298i) {
                AdvertisingIdClient advertisingIdClient = new AdvertisingIdClient(this.f25290a);
                advertisingIdClient.mo19843e();
                this.f25295f = advertisingIdClient;
            }
        } catch (GooglePlayServicesNotAvailableException | GooglePlayServicesRepairableException | IOException unused) {
            this.f25295f = null;
        }
    }

    /* JADX WARNING: Can't wrap try/catch for region: R(8:22|23|24|25|26|27|28|30) */
    /* JADX WARNING: Failed to process nested try/catch */
    /* JADX WARNING: Missing exception handler attribute for start block: B:26:0x00bd */
    /* JADX WARNING: Removed duplicated region for block: B:42:0x00d3 A[SYNTHETIC, Splitter:B:42:0x00d3] */
    /* JADX WARNING: Removed duplicated region for block: B:46:0x00da A[SYNTHETIC, Splitter:B:46:0x00da] */
    /* JADX WARNING: Removed duplicated region for block: B:54:0x00e4 A[SYNTHETIC, Splitter:B:54:0x00e4] */
    /* JADX WARNING: Removed duplicated region for block: B:58:0x00eb A[SYNTHETIC, Splitter:B:58:0x00eb] */
    /* renamed from: w */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private final void m42104w(java.io.File r11, java.lang.String r12) {
        /*
            r10 = this;
            java.lang.String r12 = "test"
            java.io.File r0 = new java.io.File
            r1 = 2
            java.lang.Object[] r2 = new java.lang.Object[r1]
            r3 = 0
            r2[r3] = r11
            r4 = 1
            java.lang.String r5 = "1658186039475"
            r2[r4] = r5
            java.lang.String r6 = "%s/%s.tmp"
            java.lang.String r2 = java.lang.String.format(r6, r2)
            r0.<init>(r2)
            boolean r2 = r0.exists()
            if (r2 == 0) goto L_0x001f
            return
        L_0x001f:
            java.io.File r2 = new java.io.File
            java.lang.Object[] r1 = new java.lang.Object[r1]
            r1[r3] = r11
            r1[r4] = r5
            java.lang.String r11 = "%s/%s.dex"
            java.lang.String r11 = java.lang.String.format(r11, r1)
            r2.<init>(r11)
            boolean r11 = r2.exists()
            if (r11 != 0) goto L_0x0037
            return
        L_0x0037:
            long r6 = r2.length()
            r8 = 0
            int r11 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r11 > 0) goto L_0x0042
            return
        L_0x0042:
            int r11 = (int) r6
            byte[] r11 = new byte[r11]
            r1 = 0
            java.io.FileInputStream r4 = new java.io.FileInputStream     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00e1, all -> 0x00ce }
            r4.<init>(r2)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00e1, all -> 0x00ce }
            int r6 = r4.read(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            if (r6 > 0) goto L_0x0058
            r4.close()     // Catch:{ IOException -> 0x0054 }
        L_0x0054:
            m42106y(r2)
            return
        L_0x0058:
            java.io.PrintStream r6 = java.lang.System.out     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r6.print(r12)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            java.io.PrintStream r6 = java.lang.System.out     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r6.print(r12)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            java.io.PrintStream r6 = java.lang.System.out     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r6.print(r12)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzand r12 = com.google.android.gms.internal.ads.zzane.m41948F()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            java.lang.String r6 = android.os.Build.VERSION.SDK     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            byte[] r6 = r6.getBytes()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzgpw r6 = com.google.android.gms.internal.ads.zzgpw.m52564R(r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r12.mo41745u(r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            byte[] r5 = r5.getBytes()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzgpw r5 = com.google.android.gms.internal.ads.zzgpw.m52564R(r5)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r12.mo41746v(r5)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzapk r5 = r10.f25293d     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            byte[] r6 = r10.f25294e     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            java.lang.String r11 = r5.mo41784a(r6, r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            byte[] r11 = r11.getBytes()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzgpw r5 = com.google.android.gms.internal.ads.zzgpw.m52564R(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r12.mo41743n(r5)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            byte[] r11 = com.google.android.gms.internal.ads.zzant.m41988e(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzgpw r11 = com.google.android.gms.internal.ads.zzgpw.m52564R(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r12.mo41744o(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r0.createNewFile()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            java.io.FileOutputStream r11 = new java.io.FileOutputStream     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            r11.<init>(r0)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cb, all -> 0x00c6 }
            com.google.android.gms.internal.ads.zzgre r12 = r12.mo47341h()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            com.google.android.gms.internal.ads.zzane r12 = (com.google.android.gms.internal.ads.zzane) r12     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            byte[] r12 = r12.mo47050p()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            int r0 = r12.length     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            r11.write(r12, r3, r0)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            r11.close()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00cc, all -> 0x00c4 }
            r4.close()     // Catch:{ IOException -> 0x00bd }
        L_0x00bd:
            r11.close()     // Catch:{ IOException -> 0x00c0 }
        L_0x00c0:
            m42106y(r2)
            return
        L_0x00c4:
            r12 = move-exception
            goto L_0x00c9
        L_0x00c6:
            r11 = move-exception
            r12 = r11
            r11 = r1
        L_0x00c9:
            r1 = r4
            goto L_0x00d1
        L_0x00cb:
            r11 = r1
        L_0x00cc:
            r1 = r4
            goto L_0x00e2
        L_0x00ce:
            r11 = move-exception
            r12 = r11
            r11 = r1
        L_0x00d1:
            if (r1 == 0) goto L_0x00d8
            r1.close()     // Catch:{ IOException -> 0x00d7 }
            goto L_0x00d8
        L_0x00d7:
        L_0x00d8:
            if (r11 == 0) goto L_0x00dd
            r11.close()     // Catch:{ IOException -> 0x00dd }
        L_0x00dd:
            m42106y(r2)
            throw r12
        L_0x00e1:
            r11 = r1
        L_0x00e2:
            if (r1 == 0) goto L_0x00e9
            r1.close()     // Catch:{ IOException -> 0x00e8 }
            goto L_0x00e9
        L_0x00e8:
        L_0x00e9:
            if (r11 == 0) goto L_0x00ee
            r11.close()     // Catch:{ IOException -> 0x00ee }
        L_0x00ee:
            m42106y(r2)
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzaqe.m42104w(java.io.File, java.lang.String):void");
    }

    /* JADX WARNING: Can't wrap try/catch for region: R(8:30|31|32|33|34|35|36|37) */
    /* JADX WARNING: Code restructure failed: missing block: B:51:0x00d8, code lost:
        r11 = th;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:52:0x00d9, code lost:
        r0 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:54:0x00dc, code lost:
        r0 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:62:?, code lost:
        r10.close();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:66:?, code lost:
        r0.close();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:73:?, code lost:
        r10.close();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:77:?, code lost:
        r0.close();
     */
    /* JADX WARNING: Failed to process nested try/catch */
    /* JADX WARNING: Missing exception handler attribute for start block: B:35:0x00c6 */
    /* JADX WARNING: Missing exception handler attribute for start block: B:47:0x00d4 */
    /* JADX WARNING: Removed duplicated region for block: B:55:? A[ExcHandler: zzapj | IOException | NoSuchAlgorithmException (unused java.lang.Throwable), SYNTHETIC, Splitter:B:12:0x004b] */
    /* JADX WARNING: Removed duplicated region for block: B:61:0x00e3 A[SYNTHETIC, Splitter:B:61:0x00e3] */
    /* JADX WARNING: Removed duplicated region for block: B:65:0x00ea A[SYNTHETIC, Splitter:B:65:0x00ea] */
    /* JADX WARNING: Removed duplicated region for block: B:72:0x00f1 A[SYNTHETIC, Splitter:B:72:0x00f1] */
    /* JADX WARNING: Removed duplicated region for block: B:76:0x00f8 A[SYNTHETIC, Splitter:B:76:0x00f8] */
    /* renamed from: x */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private final boolean m42105x(java.io.File r10, java.lang.String r11) {
        /*
            r9 = this;
            java.io.File r11 = new java.io.File
            r0 = 2
            java.lang.Object[] r1 = new java.lang.Object[r0]
            r2 = 0
            r1[r2] = r10
            r3 = 1
            java.lang.String r4 = "1658186039475"
            r1[r3] = r4
            java.lang.String r5 = "%s/%s.tmp"
            java.lang.String r1 = java.lang.String.format(r5, r1)
            r11.<init>(r1)
            boolean r1 = r11.exists()
            if (r1 != 0) goto L_0x001d
            return r2
        L_0x001d:
            java.io.File r1 = new java.io.File
            java.lang.Object[] r0 = new java.lang.Object[r0]
            r0[r2] = r10
            r0[r3] = r4
            java.lang.String r10 = "%s/%s.dex"
            java.lang.String r10 = java.lang.String.format(r10, r0)
            r1.<init>(r10)
            boolean r10 = r1.exists()
            if (r10 != 0) goto L_0x00fb
            r10 = 0
            long r5 = r11.length()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            r7 = 0
            int r0 = (r5 > r7 ? 1 : (r5 == r7 ? 0 : -1))
            if (r0 > 0) goto L_0x0043
            m42106y(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            return r2
        L_0x0043:
            int r0 = (int) r5     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            byte[] r0 = new byte[r0]     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            java.io.FileInputStream r5 = new java.io.FileInputStream     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            r5.<init>(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00ee, all -> 0x00df }
            int r6 = r5.read(r0)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            if (r6 > 0) goto L_0x0058
            m42106y(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r5.close()     // Catch:{ IOException -> 0x0057 }
        L_0x0057:
            return r2
        L_0x0058:
            com.google.android.gms.internal.ads.zzgqq r6 = com.google.android.gms.internal.ads.zzgqq.m52900a()     // Catch:{ NullPointerException -> 0x00d4, zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, zzapj | IOException | NoSuchAlgorithmException -> 0x00dc }
            com.google.android.gms.internal.ads.zzane r0 = com.google.android.gms.internal.ads.zzane.m41950H(r0, r6)     // Catch:{ NullPointerException -> 0x00d4, zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, zzapj | IOException | NoSuchAlgorithmException -> 0x00dc }
            java.lang.String r6 = new java.lang.String     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            com.google.android.gms.internal.ads.zzgpw r7 = r0.mo41750L()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r7 = r7.mo47109i()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r6.<init>(r7)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            boolean r4 = r4.equals(r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            if (r4 == 0) goto L_0x00cd
            com.google.android.gms.internal.ads.zzgpw r4 = r0.mo41748J()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r4 = r4.mo47109i()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            com.google.android.gms.internal.ads.zzgpw r6 = r0.mo41747I()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r6 = r6.mo47109i()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r6 = com.google.android.gms.internal.ads.zzant.m41988e(r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            boolean r4 = java.util.Arrays.equals(r4, r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            if (r4 == 0) goto L_0x00cd
            com.google.android.gms.internal.ads.zzgpw r4 = r0.mo41749K()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r4 = r4.mo47109i()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            java.lang.String r6 = android.os.Build.VERSION.SDK     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r6 = r6.getBytes()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            boolean r4 = java.util.Arrays.equals(r4, r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            if (r4 != 0) goto L_0x00a2
            goto L_0x00cd
        L_0x00a2:
            com.google.android.gms.internal.ads.zzapk r11 = r9.f25293d     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r4 = r9.f25294e     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            java.lang.String r6 = new java.lang.String     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            com.google.android.gms.internal.ads.zzgpw r0 = r0.mo41747I()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r0 = r0.mo47109i()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r6.<init>(r0)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            byte[] r11 = r11.mo41785b(r4, r6)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r1.createNewFile()     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            java.io.FileOutputStream r0 = new java.io.FileOutputStream     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r0.<init>(r1)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            int r10 = r11.length     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dd, all -> 0x00ca }
            r0.write(r11, r2, r10)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dd, all -> 0x00ca }
            r5.close()     // Catch:{ IOException -> 0x00c6 }
        L_0x00c6:
            r0.close()     // Catch:{ IOException -> 0x00c9 }
        L_0x00c9:
            return r3
        L_0x00ca:
            r10 = move-exception
            r11 = r10
            goto L_0x00da
        L_0x00cd:
            m42106y(r11)     // Catch:{ zzapj | IOException | NoSuchAlgorithmException -> 0x00dc, all -> 0x00d8 }
            r5.close()     // Catch:{ IOException -> 0x00d3 }
        L_0x00d3:
            return r2
        L_0x00d4:
            r5.close()     // Catch:{ IOException -> 0x00d7 }
        L_0x00d7:
            return r2
        L_0x00d8:
            r11 = move-exception
            r0 = r10
        L_0x00da:
            r10 = r5
            goto L_0x00e1
        L_0x00dc:
            r0 = r10
        L_0x00dd:
            r10 = r5
            goto L_0x00ef
        L_0x00df:
            r11 = move-exception
            r0 = r10
        L_0x00e1:
            if (r10 == 0) goto L_0x00e8
            r10.close()     // Catch:{ IOException -> 0x00e7 }
            goto L_0x00e8
        L_0x00e7:
        L_0x00e8:
            if (r0 == 0) goto L_0x00ed
            r0.close()     // Catch:{ IOException -> 0x00ed }
        L_0x00ed:
            throw r11
        L_0x00ee:
            r0 = r10
        L_0x00ef:
            if (r10 == 0) goto L_0x00f6
            r10.close()     // Catch:{ IOException -> 0x00f5 }
            goto L_0x00f6
        L_0x00f5:
        L_0x00f6:
            if (r0 == 0) goto L_0x00fb
            r0.close()     // Catch:{ IOException -> 0x00fb }
        L_0x00fb:
            return r2
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzaqe.m42105x(java.io.File, java.lang.String):boolean");
    }

    /* renamed from: y */
    private static final void m42106y(File file) {
        if (!file.exists()) {
            String.format("File %s not found. No need for deletion", new Object[]{file.getAbsolutePath()});
            return;
        }
        file.delete();
    }

    /* renamed from: z */
    private static final void m42107z(String str) {
        m42106y(new File(str));
    }

    /* renamed from: a */
    public final int mo41805a() {
        if (this.f25301l != null) {
            return zzaoz.m42028d();
        }
        return Integer.MIN_VALUE;
    }

    /* renamed from: b */
    public final Context mo41806b() {
        return this.f25290a;
    }

    /* renamed from: c */
    public final zzana mo41807c() {
        return this.f25299j;
    }

    /* renamed from: d */
    public final zzaoz mo41808d() {
        return this.f25301l;
    }

    /* renamed from: e */
    public final zzapk mo41809e() {
        return this.f25293d;
    }

    /* access modifiers changed from: package-private */
    /* renamed from: f */
    public final zzapx mo41810f() {
        return this.f25306q;
    }

    /* renamed from: h */
    public final AdvertisingIdClient mo41811h() {
        if (!this.f25296g) {
            return null;
        }
        if (this.f25295f != null) {
            return this.f25295f;
        }
        Future future = this.f25297h;
        if (future != null) {
            try {
                future.get(2000, TimeUnit.MILLISECONDS);
                this.f25297h = null;
            } catch (InterruptedException | ExecutionException unused) {
            } catch (TimeoutException unused2) {
                this.f25297h.cancel(true);
            }
        }
        return this.f25295f;
    }

    /* renamed from: i */
    public final DexClassLoader mo41812i() {
        return this.f25292c;
    }

    /* renamed from: j */
    public final Method mo41813j(String str, String str2) {
        zzarp zzarp = (zzarp) this.f25304o.get(new Pair(str, str2));
        if (zzarp == null) {
            return null;
        }
        return zzarp.mo41849a();
    }

    /* renamed from: k */
    public final ExecutorService mo41814k() {
        return this.f25291b;
    }

    /* renamed from: l */
    public final Future mo41815l() {
        return this.f25300k;
    }

    /* access modifiers changed from: package-private */
    @VisibleForTesting
    /* renamed from: o */
    public final void mo41816o(int i, boolean z) {
        if (this.f25303n) {
            Future<?> submit = this.f25291b.submit(new zzaqc(this, i, true));
            if (i == 0) {
                this.f25300k = submit;
            }
        }
    }

    /* renamed from: p */
    public final boolean mo41817p() {
        return this.f25303n;
    }

    /* renamed from: q */
    public final boolean mo41818q() {
        return this.f25302m;
    }

    /* renamed from: r */
    public final boolean mo41819r() {
        return this.f25305p;
    }

    /* renamed from: s */
    public final boolean mo41820s() {
        return this.f25306q.mo41795a();
    }

    /* renamed from: t */
    public final boolean mo41821t(String str, String str2, Class... clsArr) {
        Pair pair = new Pair(str, str2);
        if (this.f25304o.containsKey(pair)) {
            return false;
        }
        this.f25304o.put(pair, new zzarp(this, str, str2, clsArr));
        return true;
    }

    /* renamed from: u */
    public final byte[] mo41822u() {
        return this.f25294e;
    }
}
